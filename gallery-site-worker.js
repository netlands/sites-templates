...
async function fetchAndCache(url, ctx, cacheDurationSeconds = 3600) {
   ...
   */
  async function cacheContent(url, ctx, debug = false) {
    const cache = caches.default;
    const cacheKey = new Request(url);
    // Set cache duration: 1 second if debugging, otherwise 2 hours.
    const cacheDurationSeconds = debug ? 1 : 7200;
   ...
   */
  async function cacheStyling(url, ctx, debug = false) {
    const cache = caches.default;
    const cacheKey = new Request(url);
    // Set cache duration: 1 second if debugging, otherwise 24 hours.
    const cacheDurationSeconds = debug ? 1 : 86400;
   ...
   */
  async function checkLogoExistsAndCache(url, ctx, debug = false) {
    const cache = caches.default;
  ...
  export default {
    async fetch(request, env, ctx) {
      ...
      if (!useHardCodedMenu) {
        ...
        const pageListRes = await fetchAndCache(pageListUrl, ctx, debug ? 1 : 3600);
      ...
      if (path === '/') {
        ...
        // ✅ 3. Dynamic cache duration
        const response = await fetchAndCache(url.toString(), ctx, debug ? 1 : 3600);
  ...
  html = insertBeforePost(html, objectHTML);
        ...
        try {
          // ✅ 3. Dynamic cache duration
          const feedRes = await fetchAndCache(feedUrl, ctx, debug ? 1 : 3600);
  ...
  */
  ...
  html = html
    ...
    if (bodyMatch && bodyMatch[0]) {
      ...
      try {
        // ✅ 3. Pass debug flag to cache function
        inlineCSS = await cacheStyling(styleUrl, ctx, debug);
      } catch (err) {
  ...
  async function getParsedJson(url, ctx, debug = false) {
    try {
      ...
      const cacheDuration = debug ? 1 : 3600;
      const res = await fetchAndCache(fullUrl, ctx, cacheDuration);
